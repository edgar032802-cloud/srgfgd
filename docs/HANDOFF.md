...
| `public/models/terry.glb` `aqu.glb` | 돌려보기 모델 각 1MB · 30만 삼각형 · 텍스처 1024² | `wackpuball/tools/shrink-site-models.mjs` |
...
### 3D 모델 크기 — 휴대폰이 탭을 버리던 원인 (2026-09-09)

원본은 **190만 삼각형에 텍스처 4096×4096** 이었다. 문제는 다운로드(15.6MB)가 아니라 **GPU 메모리**다 — 텍스처는 GPU 에 올라갈 때 압축이 풀려 4096×4096×4B = **장당 67MB** 를 먹는다. 두 모델 여섯 장이면 400MB. 아이폰 사파리는 그 한참 아래에서 탭을 버리고 새로 고치는데, 사용자가 말한 "사용 도중 계속 새로고침된다"가 이것이다.

무대에서 캐릭터가 그려지는 크기는 350px 안팎이라 1024² 면 넉넉하다. 30만 삼각형 · 1024² 로 줄여 **15.6MB → 2.0MB, JS 힙 203MB → 76MB** 가 됐다. 모양은 실루엣으로, 색은 베이스컬러 평균값으로 확인했다(최대 차 1/255). 다시 줄여야 하면 `wackpuball` 폴더에서 `node tools/shrink-site-models.mjs`.
...
- **화면 밖이면 루프가 아무것도 하지 않는다**(2026-09-09). 예전에는 페이지가 열려 있는 내내 60fps 로 캔버스를 다시 그렸다 — 히어로를 보는 동안에도 게임이 돌고 있었다는 뜻이라 휴대폰에서 발열과 끊김이 됐다.
