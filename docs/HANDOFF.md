...
- **마감 · 마감해제 · 한국 00시 초기화**(2026-09-16, 2026-09-18 개정). **정원은 없다** — 운영자가 손으로 마감한다(푸터 로고 5번 → [마감] → 비밀번호 → 체험존 버튼 넷, 또는 운영 화면). **마감해제 = 그 체험존 줄 초기화**(대기 0, 다음 1번, 남은 대기는 `reset`). 00시에 지우는 작업은 없고 모든 조회가 "오늘 · 지금 줄 순번" 기록만 본다 — 해제는 `state.resets[날짜][체험존]` 에 시각을 하나 쌓아 순번을 올리고, 예약은 자기 `round` 를 들고 있다. 해제는 마감 상태일 때만 하므로 두 번 눌려도 한 번만 초기화된다. 날짜는 **서버 시간대를 믿지 않고 UTC+9 고정**으로 계산한다(Railway 는 UTC 라 서버 날짜를 쓰면 한국 오전 9시에 초기화된다). 자세한 규칙과 검증 목록은 `docs/RESERVATION.md` 3-1절.
- **바로 반영**(2026-09-19): 서버가 바뀌는 즉시 SSE(`/api/booth/events`)로 알리고 화면은 `src/lib/live.js` 의 `useLive` 로 따라간다(알림·다시 보일 때·주기적 확인 세 겹). 응답마다 `X-Build`(dist/build.json)가 붙어, 배포 전에 열어 둔 탭은 `src/lib/build.js` 가 안전한 때에 한 번 새로 불러온다. 화면은 `X-Client-Build` 를 싣고, 서버는 그것이 없는 옛 화면의 마감·해제를 거절한다. 자세한 것은 `docs/RESERVATION.md` 3-2절.
- **사용자 취소**(2026-09-21): 예약 화면의 [예약 취소] → `POST /api/booth/reservations/cancel {id, key}` (key = 예약 응답에서 그 기기에만 한 번 주는 `cancelKey`, 조회·운영 목록 어디에도 다시 안 나감. 공개 응답에는 이름·번호도 없다). 완료·관리자 취소와 같은 `takeOut()` 을 지나며 `cancelledBy`(user/admin)를 남긴다. 옛 취소 기록(표시 없음)은 관리자 취소로 본다.
- **검증: `npm run test:booth`** (약 6분, 문자는 나가지 않는다). 규칙 176 + 문자 경로 37 + 장면별 문자 41 + 바로 반영 46. 장면별 문자는 `node tests/booth-scenarios.test.mjs` 로 따로 돌리면 누구에게 어떤 글자가 나갔는지 그대로 찍힌다. 예약·호출 코드를 만지면 반드시 돌릴 것 — 이 부분은 독립 검토에서 경합 버그가 여럿 나왔던 곳이다.
