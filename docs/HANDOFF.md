...
- **하루 정원 100팀 · 마감 · 한국 00시 초기화**(2026-09-16). 00시에 지우는 작업은 없고 모든 조회가 "오늘" 기록만 본다 — 타이머가 없으니 자정에 서버가 꺼져 있어도 틀리지 않는다. 날짜는 **서버 시간대를 믿지 않고 UTC+9 고정**으로 계산한다(Railway 는 UTC 라 서버 날짜를 쓰면 한국 오전 9시에 초기화된다). 마감은 완료 100팀부터, 서버가 재확인한다. 자세한 규칙과 검증 목록은 `docs/RESERVATION.md` 3-1절.
- **검증: `npm run test:booth`** (약 3분, 문자는 나가지 않는다). 규칙 85 + 문자 경로 37. 예약·호출 코드를 만지면 반드시 돌릴 것 — 이 부분은 독립 검토에서 경합 버그가 여럿 나왔던 곳이다.
